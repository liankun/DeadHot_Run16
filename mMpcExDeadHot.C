...
int mMpcExDeadHot::Init(PHCompositeNode* topNode){
  ...
  hone_sensor = new TH2D("hone_sensor","One Sensor",32,-0.5,31.5,4,-0.5,3.5);
  hone_sensor->GetXaxis()->SetTitle("lx");
  hone_sensor->GetYaxis()->SetTitle("ly");
  hm->registerHisto(hone_sensor);
...
//    int quart = hit->quadrant();
    int quart = mapper->get_quadrant(key);
//    int senosr = hit->sensor_in_quadrant();
    int senosr = mapper->get_sensor_in_quadrant(key);
//    int lx = hit->lx();
...
//    unsigned short chipmap = hit->chipmap();
    ...
    if(hit->high()>0.0) hhigh_rawadc_key->Fill(key_modi,hit->high());
    if(hit->low()>0.0) hlow_rawadc_key->Fill(key_modi,hit->low());
    if(hit->high()>0.0 && calib->high_pedestal_chi2()>0) hhigh_adc_key->Fill(key_modi,hit->high()-calib->high_pedestal());
    if(hit->low()>0.0 && calib->low_pedestal_chi2()>0) hlow_adc_key->Fill(key_modi,hit->low()-calib->low_pedestal());
    double high_adc_sub = hit->high() - calib->high_pedestal();
    double low_adc_sub = hit->low() - calib->low_pedestal();
    if(high_adc_sub > 15 && calib->high_pedestal_chi2()>0){
//    if(high_adc_sub > 13){
      if(high_deadhot_list.find(key_modi) == high_deadhot_list.end()){
	hgrammyh[arm]->FillEx(key_modi,1);
	if(arm==1 && quart==1 && senosr==2 && layer==6 ) hone_sensor->Fill(lx,ly,high_adc_sub);
      }
    }
    if(low_adc_sub > 6 && calib->low_pedestal_chi2()>0){
//    if(low_adc_sub > 6){
      if(low_deadhot_list.find(key_modi) == low_deadhot_list.end()){
	hgrammyl[arm]->FillEx(key_modi,1);
      }
    }
  }
