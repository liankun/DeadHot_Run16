#include <set>
void deadhot_repass(int runNumber = 455051){
...
//  ifstream low_bad_mean("low_bad_mean.txt");
  ...
  double high_range[2][5]={{50000,50000,10000,3000,1000},
                           {3000,2000,2000,1000,500}};
  double low_range[2][5]={{50000,50000,10000,1500,1000},
                          {4000,3000,2000,300,100}};
...
//    cout<<key<<" "<<new_key<<endl;
  ...
  for(int i = 0;i < 24576*2;i++){
    ...
    /*
    if(htemp0->GetEntries() < 10){
      if(htemp1->GetEntries() >= 10){
        cout<<"strange"<<endl;
	cout<<"high gian entries: "<<htemp0->GetEntries()
	    <<" low gain "<<htemp1->GetEntries()
	    <<" key: "<<i<<endl;
      }
      continue;
    }
    */
    ...
    if(bad_high_itg_list.find(i) != bad_high_itg_list.end()){
      int maximum_bin = htemp0->GetMaximumBin();
      double bin_center = htemp0->GetBinCenter(maximum_bin);
      high_shift = bin_center;
    }

    if(bad_low_itg_list.find(i) != bad_low_itg_list.end()){
      int maximum_bin = htemp1->GetMaximumBin();
      double bin_center = htemp1->GetBinCenter(maximum_bin);
      low_shift = bin_center;
    }
