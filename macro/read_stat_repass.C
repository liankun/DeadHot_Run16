#include<fstream>
void read_stat_repass(){
  TFile* ifile = new TFile("deadhot_statistic_repass.root","READONLY");
...
//  TH1D* hlow_deadhot_runNum = (TH1D*)ifile->Get("hlow_deadhot_runNum");
  ofstream high_deadhot_stat("high_deadhot_stat_repass.txt");
  ofstream low_deadhot_stat("low_deadhot_stat_repass.txt");
  ofstream high_real_deadhot("high_real_deadhot_repass.txt");
  ofstream low_real_deadhot("low_real_deadhot_repass.txt");
  ...
  TFile* ofile = new TFile("statistic_repass.root","RECREATE");
  hstat_high->Write();
  hstat_low->Write();
}
