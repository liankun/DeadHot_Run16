#include<map>
#include<vector> 
void readitg_r(int runNumber=455051){
...
//cout<<size<<endl;
	...
	for(int j = 0;j<size;j++){
	  ...
	  if(bin_content0 > mean0[n]+5.*rms0[n] || bin_content0 < mean0[n]-5.*rms0[n]){
            bad_high_itg<<key_to_key[new_key]<<" "<<new_key<<endl;
	  }
	  if(bin_content1 > mean1[n]+5.*rms1[n] || bin_content1 < mean1[n]-5.*rms1[n]){
	    bad_low_itg<<key_to_key[new_key]<<" "<<new_key<<endl;
	  }
	}
      }//ilayer
