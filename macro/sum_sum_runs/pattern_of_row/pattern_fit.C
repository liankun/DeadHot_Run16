...
void pattern_fit(){
  ...
  ofstream high_out_txt("fit_high_linear.txt");
  ofstream low_out_txt("fit_low_linear.txt");
  for(int iitg=1;iitg<5;iitg++){
    for(int iarm=0;iarm < 2;iarm++){
  ...
  TFile* ofile = new TFile("work_2016_09_07/output_cut.root","RECREATE");
  for(int iarm = 0;iarm < 2;iarm++){
    for(int iitg=1;iitg<5;iitg++){
