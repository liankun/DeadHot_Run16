...
void pattern_fit_pol2_on_nocut(){
  TFile* ifile = new TFile("Integral_deadhot_deadhot_fit_50p_cut.root");  
  TH1D* hhigh_itg[2][5];
  TH1D* hlow_itg[2][5];
...
//  ofstream low_out_txt("fit_low_pol2_refit.txt");
  ...
  TFile* ofile = new TFile("work_2016_09_12/output_fit_50p_cut_pol2_with_fitcut.root","RECREATE");
  for(int iarm = 0;iarm < 2;iarm++){
    for(int iitg=1;iitg<5;iitg++){
