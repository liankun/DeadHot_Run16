...
void pattern_fit_pol2_on_nocut(){
  ...
  TFile* ifile = new TFile("Integral_deadhot_deadhot_fit_40p_cut.root");  
  TH1D* hhigh_itg[2][5];
  TH1D* hlow_itg[2][5];

  TH1D* hfit_chi2[2][5];

  ifstream in_bad_high_txt("ashley_high_only.txt");
  ifstream in_bad_low_txt("ashley_low_only.txt");
