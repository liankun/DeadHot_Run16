...
void pattern_fit2(){
  ...
  ifstream high_out_txt("fit_high_linear.txt");
  ifstream low_out_txt("fit_low_linear.txt");
  ...
  while(high_out_txt>>in_arm>>in_iitg>>in_row>>in_par0>>in_par1){
    high_par0[in_arm][in_iitg][in_row]=in_par0;
    high_par1[in_arm][in_iitg][in_row]=in_par1;
  }
  while(low_out_txt>>in_arm>>in_iitg>>in_row>>in_par0>>in_par1){
    low_par0[in_arm][in_iitg][in_row]=in_par0;
    low_par1[in_arm][in_iitg][in_row]=in_par1;
  }
  ...
  for(int iitg=1;iitg<5;iitg++){
	...
	sprintf(fname,"fit_arm%d_itg%d_row%d",iarm,iitg,i);
        TF1* fit = new TF1(fname,fit0,32*i+24576*iarm,i*32+31+24576*iarm,2);
  ...
  TFile* ofile = new TFile("work_2016_09_07/output_no_cut.root","RECREATE");
  for(int iarm = 0;iarm < 2;iarm++){
    for(int iitg=1;iitg<5;iitg++){
