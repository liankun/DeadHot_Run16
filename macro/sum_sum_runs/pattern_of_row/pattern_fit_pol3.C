...
void pattern_fit_pol3(){
  ...
  ofstream high_out_txt("fit_high_pol3.txt");
  ofstream low_out_txt("fit_low_pol3.txt");
  for(int iitg=1;iitg<5;iitg++){
    for(int iarm=0;iarm < 2;iarm++){
      for(int i = 0;i < 192*4;i++){
        char fname[500];
	sprintf(fname,"fit_arm%d_itg%d_row%d",iarm,iitg,i);
        TF1* fit = new TF1(fname,fit0,32*i+24576*iarm,i*32+31+24576*iarm,4);
  ...
  TFile* ofile = new TFile("work_2016_09_07/output_cut_pol3.root","RECREATE");
  for(int iarm = 0;iarm < 2;iarm++){
    for(int iitg=1;iitg<5;iitg++){
