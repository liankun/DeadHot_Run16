...
#include<fstream>
#include <iomanip>
void pattern_fit_pol2(){
  TFile* ifile = new TFile("Integral_deadhot_deadhot_previous_cut.root");  
  ...
  ofstream high_out_txt("fit_high_pol2_new2.txt");
  ofstream low_out_txt("fit_low_pol2_new2.txt");
  for(int iitg=1;iitg<5;iitg++){
    for(int iarm=0;iarm < 2;iarm++){
	...
	high_out_txt<<iarm<<" "<<iitg<<" "<<i<<" "
	       <<setprecision(11)<<fit->GetParameter(0)<<" "
	       <<setprecision(11)<<fit->GetParameter(1)<<" "
	       <<setprecision(11)<<fit->GetParameter(2)<<" "
	       <<fit->GetNDF()
	       <<endl;
  ...
  TFile* ofile = new TFile("output_cut_pol2_Lfit.root","RECREATE");
  for(int iarm = 0;iarm < 2;iarm++){
    for(int iitg=1;iitg<5;iitg++){
