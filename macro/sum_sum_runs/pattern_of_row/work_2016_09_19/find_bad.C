...
void find_bad(){
  TFile* ifile = new TFile("../pedestal_affect/Integral_deadhot_deadhot_no_cut.root");
  ...
  ifstream high_in_txt("fit_high_pol2_refit2.txt");
  ifstream low_in_txt("fit_low_pol2_refit2.txt");
  ...
  ofstream high_bad_txt("high_bad_up_range_list.txt");
  ofstream low_bad_txt("low_bad_up_range_list.txt");

  set<int> high_bad_set;
  set<int> low_bad_set;
  for(int iitg=3;iitg<5;iitg++){
    for(int iarm=0;iarm < 2;iarm++){
	...
	for(int j = 32*i;j<32*(i+1);j++){
	  double bin_content0=hhigh_itg[iarm][iitg]->GetBinContent(j+1);
	    ...
	    if(fabs((bin_content0-eval_content0)/eval_content0)>0.4){
              if(high_bad_set.find(24576*iarm+j)==high_bad_set.end()){
	        high_bad_txt<<key_to_key[24576*iarm+j]<<endl;
	        high_bad_set.insert(24576*iarm+j);
	      }
	  ...
	  if(bin_content1<10){
	    ...
	    if(fabs((bin_content1-eval_content1)/eval_content1)>0.4){
	      if(low_bad_set.find(24576*iarm+j)==low_bad_set.end()){
                low_bad_txt<<key_to_key[24576*iarm+j]<<endl;
	        low_bad_set.insert(24576*iarm+j);
	      }
  ...
  TFile* ofile = new TFile("diff.root","RECREATE");
  for(int iitg=1;iitg<5;iitg++){
    for(int iarm=0;iarm < 2;iarm++){
