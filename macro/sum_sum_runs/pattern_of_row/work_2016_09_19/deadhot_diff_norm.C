...
//  sprintf(path,"%d/sum_sum.root",runNumber);
  ...
  ifstream bad_high_itg_total("high_bad_list.txt");
  
  ifstream bad_low_itg_total("low_bad_list.txt");
  ...
  double low_itg_begin[5] = {0,9,15,25,40};
...
//  Double_t rbins[10] ={6.5,9,10.5,11.9,13.5,14.8,15.8,16.8,18.0,21.5};
  ...
  ifstream high_out_txt("fit_high_pol2_refit2.txt");
  ifstream low_out_txt("fit_low_pol2_refit2.txt");
  ...
  while(high_out_txt>>iarm>>iitg>>irow>>par0>>par1>>par2>>ndf){  
    high_par0[iarm][iitg][irow]=par0;                  
    high_par1[iarm][iitg][irow]=par1;                  
    high_par2[iarm][iitg][irow]=par2;                
  }     
                                                            
  while(low_out_txt>>iarm>>iitg>>irow>>par0>>par1>>par2>>ndf){   
...
//    if(htemp0->GetEntries() < 10) continue;
  ...
  TFile* ofile = new TFile("Integral_deadhot_deadhot_refit_cut_norm.root","RECREATE");
  for(int iarm = 0;iarm < 2;iarm++){
    for(int iitg = 1;iitg<5;iitg++){
