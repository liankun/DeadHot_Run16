...
//  sprintf(path,"%d/sum_sum.root",runNumber);
  ...
  ifstream bad_high_itg_total("44/high_deadhot_row_pattern.txt");
  
  ifstream bad_low_itg_total("44/low_deadhot_row_pattern.txt");
...
//    if(htemp0->GetEntries() < 10) continue;
  ...
  TFile* ofile = new TFile("Integral_deadhot_deadhot_row_cut.root","RECREATE");
  for(int iarm = 0;iarm < 2;iarm++){
