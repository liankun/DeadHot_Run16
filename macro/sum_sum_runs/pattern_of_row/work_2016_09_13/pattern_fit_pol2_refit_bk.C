#include<map>
#include<fstream>
void pattern_fit_pol2_refit(){
  TFile* ifile = new TFile("../work_2016_09_12/Integral_deadhot_deadhot_row_cut.root");  
  TH1D* hhigh_itg[2][5];
  TH1D* hlow_itg[2][5];
  ...
  ofstream high_out_txt("fit_high_pol2_refit2.txt");
  ofstream low_out_txt("fit_low_pol2_refit2.txt");
  ...
  for(int iitg=1;iitg<5;iitg++){
	...
	sprintf(fname,"fit_arm%d_itg%d_row%d",iarm,iitg,i);
        ...
        int tmp_ndf=high_ndf[iarm][iitg][i];
	...
	if(low_ndf[iarm][iitg][i]>11){
	  ...
	  tmp_ndf=fit->GetNDF();
	  if(i==124 && iarm==0){
            TF1* fit_test = new TF1("fit_test",fit0,32*i+24576*iarm,i*32+31+24576*iarm,3);
	    double a=fit->GetParameter(0);
	    double b=fit->GetParameter(1);
	    double c=fit->GetParameter(2);
	    fit_test->FixParameter(0,a);
	    fit_test->FixParameter(1,b);
	    fit_test->FixParameter(2,c);
	    cout<<low_par0[iarm][iitg][i]<<" "
	      <<low_par1[iarm][iitg][i]<<" "
	      <<low_par2[iarm][iitg][i]<<" "
	      <<low_ndf[iarm][iitg][i]
	      <<endl;
	    cout<<fit->GetParameter(0)<<" "
	      <<fit->GetParameter(1)<<" "
	      <<fit->GetParameter(2)<<" "
	      <<fit->GetNDF()<<" "
	      <<fit->Eval(3990)<<" "
  ...
  TFile* ofile = new TFile("output_row_cut_pol2_with_cut_refit_ndf_11.root","RECREATE");
  for(int iarm = 0;iarm < 2;iarm++){
    for(int iitg=1;iitg<5;iitg++){
