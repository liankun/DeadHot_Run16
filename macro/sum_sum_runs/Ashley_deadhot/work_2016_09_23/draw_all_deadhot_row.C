...
void draw_all_deadhot_row(int group=0){
  ...
  ifstream bad_high_itg_total("../high_bad_list_merge.txt");
  ifstream bad_low_itg_total("../low_bad_list_merge.txt");

  ifstream high_deadhot_output("../ashley_high_only.txt");
  ifstream low_deadhot_output("../ashley_low_only.txt");

  ifstream high_deadhot_redefine("../high_deadhot_reiden.txt");

  char type[100]="ashley_only_redefine";

  std::set<int> bad_high_itg_list;
  std::set<int> bad_low_itg_list;
  std::set<int> bad_high_output_set;
  std::set<int> bad_low_output_set;
  std::set<int> bad_high_redefine_set;
  ...
  double low_itg_begin[6] = {0,9,15,25,40,100};
  double low_itg_end[5] = {9,15,25,40,100};
  ...
  double low_itg_region[8]={-40.5,0,9,15,25,40,100,259.5};
  ...
  while(high_deadhot_redefine>>key){
    bad_high_redefine_set.insert(key);
  }
  ...
  TFile* ofile = new TFile("all_deadhot_adc_liankun.root","RECREATE");


//  ofstream high_output_info("work_2016_09_21/high_output_info_ashley_only.txt");
  int num_hist=1;
  int num_c=0;
  char c_name[500];
...
//    phigh_adc_r[r_index]->Draw("");
    if(bad_high_redefine_set.find(key)==bad_high_redefine_set.end()){
      phigh_adc_row[row_num]->Draw("");
      htemp0->Draw("same");
    }
    tp_list.push_back(htemp0);
    
//    high_output_info<<key<<" "<<new_key<<" "<<num_c<<" "<<num_hist<<endl;
//    delete ptemp;
//    delete htemp0;
    num_hist++;
    if(num_hist>100){
      c->Write();
      sprintf(c_name,"c_high_num%d_%s.gif",num_c,type);
      c->Print(c_name,"gif");
      num_c++;
      sprintf(c_name,"c_high_num%d",num_c);
      TCanvas* c = new TCanvas(c_name,c_name,1200,800);
...
//    low_output_info<<key<<" "<<num_c<<" "<<num_hist<<endl;
 ...
 //   tp_list.push_back(htemp);
    if(num_hist>100){
      c->Write();
      sprintf(c_name,"c_low_num%d_%s.gif",num_c,type);
      c->Print(c_name,"gif");
      num_c++;
      sprintf(c_name,"c_low_num%d",num_c);
      TCanvas* c = new TCanvas(c_name,c_name,1200,800);
