...
void draw_all_deadhot(int group=0){
  ...
  char type[100]="ashley_only_with_liankun_cut";
  ...
  TFile* ofile = new TFile("work_2016_09_16/all_deadhot_adc_liankun.root","RECREATE");
...
//    delete htemp0;
    ...
    if(num_hist>100){
      c->Write();
      sprintf(c_name,"work_2016_09_16/c_high_num%d_%s.gif",num_c,type);
      c->Print(c_name,"gif");
      num_c++;
      sprintf(c_name,"c_high_num%d",num_c);
      TCanvas* c = new TCanvas(c_name,c_name,1200,800);
...
//    low_output_info<<key<<" "<<num_c<<" "<<num_hist<<endl;
 ...
 //   tp_list.push_back(htemp);
    if(num_hist>100){
      c->Write();
      sprintf(c_name,"work_2016_09_16/c_low_num%d_%s.gif",num_c,type);
      c->Print(c_name,"gif");
      num_c++;
      sprintf(c_name,"c_low_num%d",num_c);
      TCanvas* c = new TCanvas(c_name,c_name,1200,800);
