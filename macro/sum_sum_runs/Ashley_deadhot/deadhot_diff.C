...
//  ifstream bad_high_itg_total(path);
  ...
  ifstream bad_high_itg_total("ashley_high_deadhot.txt");
  
//  sprintf(path,"%d/low_deadhot.txt",runNumber);
//  ifstream bad_low_itg_total(path);

  ifstream bad_low_itg_total("ashley_low_deadhot.txt");
...
//  sprintf(path,"%d/Integral_deadhot_ashely.root",runNumber);
  TFile* ofile = new TFile("work_2016_09_16/Integral_deadhot_deadhot_ashely.root","RECREATE");
  for(int iarm = 0;iarm < 2;iarm++){
