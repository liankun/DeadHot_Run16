...
void draw_deadhot_adc(){
  ifstream high_deadhot_txt("same_high_only2.txt");
  ifstream low_deadhot_txt("same_low_only2.txt");
...
cout<<"high "<<key<<endl;
    ...
    TH1D* htemp = hhigh_rawadc_key->ProjectionY();
...
cout<<"low "<<key<<endl;
    ...
    TH1D* htemp = hlow_rawadc_key->ProjectionY();
    int max_bin = htemp->GetMaximumBin();
    double max_center = htemp->GetBinCenter(max_bin);
    double rms = htemp->GetRMS();
//    if(rms < 1) continue;
...
//    low_bad_ped<<key<<" "<<max_center<<endl;
  ...
  TFile* ofile = new TFile("work_2016_07_22/bad_channel_adc_same2.root","RECREATE");
  hhigh_bad_adc->Write();
