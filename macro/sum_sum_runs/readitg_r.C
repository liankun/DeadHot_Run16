#include<map>
#include<vector> 
void readitg_r(int runNumber=0){
...
//	  rms0[k] = rms0[k]/sqrt(count0[k]);
//	  rms1[k] = rms1[k]/sqrt(count1[k]);
	...
	for(int j = 0;j<size;j++){
	  ...
	  if(n<ndiv-1){
            if(bin_content0 < mean0[n+1]-3*rms0[n+1] || bin_content0 > mean0[n+1]+6*rms0[n+1]){
              bad_high_itg<<key_to_key[new_key]<<" "<<new_key<<endl;  
	    }
            if(bin_content1 < mean1[n+1]-3*rms1[n+1] || bin_content1 > mean1[n+1]+6*rms1[n+1]){
              bad_low_itg<<key_to_key[new_key]<<" "<<new_key<<endl;
	    }
	  }
