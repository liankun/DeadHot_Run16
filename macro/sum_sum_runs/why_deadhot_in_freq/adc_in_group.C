...
void adc_in_group(int group=38,int want_key=3729){
...
//  double high_itg_region_type1[2][8]={{-40.5,0,15,45,80,120,200,259.5},
//                                      {-40.5,0,15,30,50,80,150,259.5}};
  ...
  sprintf(path,"deadhot_adc_row_key%d_group%d.root",want_key,group);
  TFile* ofile = new TFile(path,"RECREATE");
  
  bool is_want_key=false;
  ...
  int arm = mapper->get_arm(want_key);
  int layer = mapper->get_layer(want_key);
cout<<"key: "<<want_key<<" arm "<<arm<<" layer "<<layer<<endl;
  int quadrant = mapper->get_quadrant(want_key);
  int sensor = mapper->get_sensor_in_quadrant(want_key);
  int index = 6*quadrant+sensor;
  int ly = mapper->get_ly(want_key);
  int row_num=192*4*arm+24*4*layer+index*4+ly;
  double pos_x = mapper->get_x(want_key);
  double pos_y = mapper->get_y(want_key);
  double r = sqrt(pos_x*pos_x+pos_y*pos_y);
  int n = -1;
  ...
  int r_index=9*8*arm+9*layer+n;
  hhigh_adc_key->SetAxisRange(want_key,want_key,"x");
  TH1D* htemp0 = hhigh_adc_key->ProjectionY();
  ...
  char hname[500];
  sprintf(hname,"c_high_key%d_deadhot%d",want_key,(int)is_want_key);
  ...
  int r_index=9*8*arm+9*layer+n;
  hlow_adc_key->SetAxisRange(want_key,want_key,"x");
  ...
  char hname[500];
  sprintf(hname,"c_low_key%d_deadhot%d",want_key,(int)is_want_key);
  TCanvas* c = new TCanvas(hname,hname,800,1200);
  c->Divide(2,2);
  c->cd(1);
  gPad->SetLogy();
