...
//  ifstream bad_low_itg_total(path);
  ...
  ifstream bad_high_itg_total("why_deadhot_in_freq/high_want_deadhot_40_44.txt");
  ifstream bad_low_itg_total("why_deadhot_in_freq/low_want_deadhot_40_44.txt");
  ...
  for(int i = 0;i<1536;i++){
    ...
    if(layer>1){
      phigh_itg_row[i] = new TProfile(pname,pname,7,high_itg_region_type1[arm]);
    }
    else{
      phigh_itg_row[i] = new TProfile(pname,pname,7,high_itg_region_type0[arm]);
    }
    phigh_itg_row[i]->GetXaxis()->SetTitle("region");
    sprintf(pname,"plow_itg_row%d",i);
    plow_itg_row[i] = new TProfile(pname,pname,7,low_itg_region);
  ...
  for(int i=0;i<144;i++){
    ...
    if(layer>1){
      phigh_itg_r[i] = new TProfile(pname,pname,7,high_itg_region_type1[arm]);
    }
    else{
      phigh_itg_r[i] = new TProfile(pname,pname,7,high_itg_region_type0[arm]);
    }
    phigh_itg_r[i]->GetXaxis()->SetTitle("region");
    sprintf(pname,"plow_itg_r%d",i);
    plow_itg_r[i] = new TProfile(pname,pname,7,low_itg_region);
