...
#include <map>
#include <fstream>
void deadhot_repass(int runNumber = 0){
...
//  TFile* file = new TFile("all_run.root","READONLY");
...
//  ifstream bad_high_itg_total("work_2016_06_22/high_real_deadhot_27cut.txt");
//  ifstream bad_low_itg_total("work_2016_06_22/low_real_deadhot_27cut.txt");

  
  ifstream high_bad_ped("high_bad_ped.txt");
  ifstream low_bad_ped("low_bad_ped.txt");
  ...
  std::map<int,double> high_ped_list;
  std::map<int,double> low_ped_list;
...
//    cout<<key<<" "<<new_key<<endl;
  ...
  while(high_bad_ped>>key>>ped){
//    cout<<key<<" "<<ped<<endl;
    high_ped_list.insert(std::pair<int,double>(key,ped));
  }

  while(low_bad_ped>>key>>ped){
//    cout<<key<<" "<<ped<<endl;
    low_ped_list.insert(std::pair<int,double>(key,ped));
  }
...
//    if(htemp0->GetEntries() < 10) continue;
    ...
    for(int iitg = 0;iitg<5;iitg++){
      ...
      std:;map<int,double>::iterator it = high_ped_list.find(i);
      if(it != high_ped_list.end()){
        high_begin = high_begin+it->second;
	high_end = high_end+it->second; 
      }
  ...
  sprintf(path,"%d/Integral_repass.root",runNumber);
