...
void read_deadhot(){
  ...
  TH1D* hhigh_deadhot_runNum = new TH1D("hhigh_deadhot_runNum","high dead hot number vs group ",44,-0.5,43.5);
  hhigh_deadhot_runNum->GetXaxis()->SetTitle("runNumber");

  TH1D* hlow_deadhot_runNum = new TH1D("hlow_deadhot_runNum","low dead hot number vs run number",44,-0.5,43.5);
...
//        if(key<24576)continue;
        count++;
//	if(key==49062) cout<<runNumber<<endl;
  ...
  TFile* ofile = new TFile("deadhot_statistic_add_discrete_loose_cut.root","RECREATE");
