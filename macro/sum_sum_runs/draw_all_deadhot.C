...
void draw_all_deadhot(int group=0){
  ...
  ifstream bad_high_itg_total("high_real_deadhot_add_discrete_large_adc.txt");
  ifstream bad_low_itg_total("good_low_deadhot_large_adc.txt");
  ...
  TFile* ofile = new TFile("all_deadhot_adc.root","RECREATE");
...
//    delete htemp0;
    ...
    if(num_hist>100){
      c->Write();
      sprintf(c_name,"work_2016_08_04/c_high_num%d.gif",num_c);
      c->Print(c_name,"gif");
      num_c++;
      sprintf(c_name,"c_high_num%d",num_c);
      TCanvas* c = new TCanvas(c_name,c_name,1200,800);
...
//    tp_list.clear();
 ...
 //   tp_list.push_back(htemp);
    if(num_hist>100){
      c->Write();
      sprintf(c_name,"work_2016_08_04/c_low_num%d.gif",num_c);
      c->Print(c_name,"gif");
      num_c++;
      sprintf(c_name,"c_low_num%d",num_c);
      TCanvas* c = new TCanvas(c_name,c_name,1200,800);
