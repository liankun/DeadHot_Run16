...
*/
	...
	for(int j = 32*i;j<32*(i+1);j++){
	  ...
	  double bin_content1 = hlow_itg[iarm][iitg]->GetBinContent(j+1);
          ...
          if(bin_content0 > 1 && count0 > 1){
	    double mean0_single=(mean0-bin_content0)/(count0-1);
	    double sq_mean0_single=(sq_mean0-bin_content0*bin_content0)/(count0-1);
	    double rms0 = sqrt(sq_mean0_single-mean0_single*mean0_single);
	    double rms0 = rms0/sqrt(count0-1);
            
	    if(bin_content0 > mean0_single+16*rms0 || bin_content0 < mean0_single-16*rms0){
              int new_key = 24576*iarm+j;
              bad_high_itg<<key_to_key[new_key]<<" "<<new_key<<endl;
	    }
	  }
	  
	  if(bin_content1 > 1 && count1 > 1){
	    double mean1_single = (mean1-bin_content1)/(count1-1);
	    double sq_mean1_single = (sq_mean1-bin_content1*bin_content1)/(count1-1);
	    double rms1 = sqrt(sq_mean1_single-mean1_single*mean1_single);
            double rms1 = rms1/sqrt(count1-1);

	    if(bin_content1 > mean1_single+16*rms1 || bin_content1 < mean1_single-16*rms1){
              int new_key = 24576*iarm+j;
	      bad_low_itg<<key_to_key[new_key]<<" "<<new_key<<endl;
	    }
	  }
